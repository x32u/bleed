...
class EmojiFinder(Converter):
    ...
    async def convert(ctx: Context, argument: str):
        ...
        # Check if input looks like a Discord emoji URL/ID
        if any(x in argument for x in ("discord", "emojis", ".gif", ".png")):
            raise CommandError(f"`{argument}` is not a **valid emoji**")

        # Handle Unicode emoji
        try:
            # Convert to hex and filter out variation selectors
            characters = []
            ...
            url = f"https://cdn.jsdelivr.net/gh/twitter/twemoji/assets/72x72/{hexcode}.png"
            ...
            raise CommandError(f"`{argument}` is not a **valid emoji**") from e
